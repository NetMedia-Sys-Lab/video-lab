...
class HeadlessPlayerApi:
    ...
    def init_routes(self):
        ...
        def _start_new_experiment():
            ...
            return jsonify(
                {"success": True, "message": f"Successfully Scheduled {len(scheduled_runs)} runs.", "runs": scheduled_runs}
            )
        ...
        def _get_encode_playback():
            ...
            for run_id in run_ids:
                job = self.job_manager.schedule(
                    PythonJob(
                        config={"callback": Codec.encode_playback_job.__name__, "args": (run_id,), "kwargs": {}, "name": None}
                    )
                )
                job_ids.append(job.job_id)
            return jsonify({"message": f"Scheduled {len(run_ids)} playback encoding", "job_ids": job_ids})
        ...
        def _get_quality():
            ...
            for run_id in run_ids:
                job = self.job_manager.schedule(
                    PythonJob(
                        config={"callback": Codec.calculate_quality.__name__, "args": (run_id,), "kwargs": {}, "name": None}
                    )
                )
                job_ids.append(job.job_id)
        ...
        def _get_run_detail():
            ...
            data = {run: get_run(run).json() for run in runs}
        ...
        def _open_wireshark():
            ...
            p1 = subprocess.Popen(
                ["mergecap", "-w", "-", *[f"{CONFIG['headlessPlayer']['resultsDir']}/{f}" for f in files]], stdout=subprocess.PIPE
            )
        ...
        def create_tiles():
            ...
            self.job_manager.schedule(
                PythonJob(config={"callback": Codec.create_tiles_video.__name__, "args": (run_ids,), "kwargs": {}})
            )
