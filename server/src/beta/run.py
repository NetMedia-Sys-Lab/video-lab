...
class Run:
    ...
    def run_config(self) -> RunConfig:
        run_config_file = join(results_dir, self.result,
    ...
    @property
    def events(self):
        event_log_file = join(results_dir, self.result,
                              self.run, "event_logs.txt")
    ...
    def vmaf(self):
        ...
        for index in self.segments_df.index:
            ...
            vmaf_files.append(
                join(self.vmaf_dir, f"vmaf-stream{quality}-{index+1:05d}.json"))
        ...
        def result():
            ...
            return {
                "segments": vmafs,
                "mean": np.mean(list(filter(bool, vmafs)) or [0])
            }
        return result()
    ...
    def micro_stalls(self):
        ...
        for index in self.segments_df.index:
                quality = self.segments_df['quality'][index]
                stall_files.append(join(self.frames_dir,
                                f"frames-stream{quality}-{index+1:05d}.json"))
